...
class DataValidation:
    ...
    def contains_numerical_columns(self,dataframe:pd.DataFrame):
        try:
            for col in self._schema_config["numerical_columns"]:
                if not is_numeric_dtype(dataframe[col]):
                    logging.info(f"{col} is not numerical")
                    return False
            return True
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def initiate_data_validation(self):
        try:
            ...
            status = self.contains_numerical_columns(train_dataframe)
            if not status:
                logging.info("Train dataframe does not contain all numerical columns.")
            status = self.contains_numerical_columns(test_dataframe)
            if not status:
                logging.info("Test dataframe does not contain all numerical columns.")
